...
DEFAULT_DATASET_ROOT = Path(
    r"C:\Users\Staff\OneDrive - University of Greenwich\HandsDatasets\archive"
)
DEFAULT_IMAGES_DIR_NAME = "Photos"
DEFAULT_MASKS_DIR_NAME = "Masks"
DEFAULT_CSV_NAME = "annotated_dataset_details.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate binary hand masks for the archive dataset by thresholding "
    ...
    parser.add_argument(
        ...
        help="Root folder that contains annotated_dataset_details.csv and Photos/.",
    )
    parser.add_argument(
        "--csv",
        type=Path,
        default=None,
        help="Path to annotated_dataset_details.csv (defaults to <dataset-root>/annotated_dataset_details.csv).",
    )
    parser.add_argument(
        "--images-dir",
        type=Path,
        default=None,
        help="Path to images folder (defaults to <dataset-root>/Photos).",
    )
    parser.add_argument(
        "--masks-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Comma-separated list of extensions to try when imageKey has no suffix.",
    )
    parser.add_argument(
        "--bg-method",
        ...
        default="border",
        help=(
            "Background detection method. 'simple' uses a fixed black threshold. "
    ...
    parser.add_argument(
        ...
        default=40,
        help=(
            "Maximum channel value to treat a pixel as black background (0-255). "
    ...
    parser.add_argument(
        ...
        default=30,
        help=(
            "Max channel spread to still count as black (max - min). "
            "Used with --bg-method simple."
        ),
    )
    parser.add_argument(
        ...
        type=float,
        default=95.0,
        help="Percentile of border distance-to-black to classify background in border mode.",
    )
    parser.add_argument(
        "--border-margin",
        type=int,
    ...
    parser.add_argument(
        "--preview",
        action="store_true",
        help="Show each image with its generated mask (close the window to advance, press q to quit).",
    )
    parser.add_argument(
        "--preview-limit",
        type=int,
...
def _parse_bbox_values(bbox_str: str) -> Optional[Tuple[int, int, int, int]]:
    if not bbox_str:
        return None
    parts = [p.strip().strip('"') for p in bbox_str.split(",")]
    if len(parts) != 4:
        return None
    try:
...
def _build_background_mask_simple(
    image_bgr: np.ndarray, black_max: int, black_tolerance: int
) -> np.ndarray:
    bgr = image_bgr.astype(np.int16)
    min_c = bgr.min(axis=2)
    max_c = bgr.max(axis=2)
    return (max_c <= black_max) & ((max_c - min_c) <= black_tolerance)
...
def _build_background_mask_border(
    ...
    black_lab = np.array([0.0, 128.0, 128.0], dtype=np.float32)
    dist = np.linalg.norm(lab - black_lab, axis=2)
...
def main() -> None:
    ...
    with csv_path.open(newline="", encoding="utf-8-sig") as csv_file:
        ...
        if reader.fieldnames is None or "imageKey" not in reader.fieldnames:
            raise ValueError("CSV must contain an imageKey column.")
        ...
        for row in reader:
            ...
            image_key = (row.get("imageKey") or "").strip()
            image_path = _resolve_image_path(image_key, images_dir, extensions)
            if image_path is None:
                print(f"[WARN] Image not found for imageKey='{image_key}'")
                continue
            ...
            if args.bg_method == "simple":
                bg_mask = _build_background_mask_simple(
                    roi, args.black_max, args.black_tolerance
                )
            else:
                bg_mask = _build_background_mask_border(
